    ...
    constructor(username: string, password: string) {
      ...
      getUserNames(): Cypress.Chainable<string[]> {
        return cy.get('.login_credentials').then($el => {
          ...
          const parts = rawHtml.split('<br>').map(part => part.trim()).filter(part => part.length > 0);
          // Prvý element môže byť nadpis, odstránime ho, ak tam je
          const filtered = parts.filter(text => !text.toLowerCase().includes('accepted usernames'));
